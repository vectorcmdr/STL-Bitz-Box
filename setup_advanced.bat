...
title STL Bitz Box Setup - Main Menu
...
:MainMenu
cls
echo STL Bitz Box Setup ^& Tools^:!ESC![^?25l
echo\
echo Press the number key to make a choice.
echo\
echo INSTALL:
echo 1. Install STL Thumbnail Dependencies
echo 2. Install Server
echo\
echo SETUP:
echo 3. Convert OBJs to STLs
echo 4. Rebuild Thumbnails ^& DB
echo\
echo\
...
echo 6. Rebuild Thumbnails ONLY
echo 7. Rebuild DB ONLY
echo 8. Library File Size Audit
echo 9. FBX^/OBJ File Audit
echo\
echo\

choice /C 123456789 /M "Enter choice to continue:"

if errorlevel 9 goto FileTypeAudit
if errorlevel 8 goto FileSizeAudit
if errorlevel 7 goto RebuildDB
if errorlevel 6 goto RebuildThumbs

if errorlevel 5 goto ExitMenu

if errorlevel 4 goto RebuildAll
if errorlevel 3 goto OBJtoSTL
if errorlevel 2 goto InstallServer
if errorlevel 1 goto InstallThumbs
...
:InstallServer
cls
	title STL Bitz Box Setup - Server Installer
...
:InstallThumbs
cls
	title STL Bitz Box Setup - PNG Thumbnail Support Installer
...
:FileSizeAudit
cls
	title STL Bitz Box Setup - File Size Audit . . .
	
	set "reportfsize=reports\Report_FileSize"
	del "!reportfsize!.txt" >nul 2>&1
	
	set /a "fiftymbcount=0"
	set /a "hundredmbcount=0"

	set "spinner=|"

	echo Beginning file size audit^.

	echo Files found at or over 50MB^: >> !reportfsize!.txt
	echo ^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^= >> !reportfsize!.txt
	echo\  >> !reportfsize!.txt
	...
	for /r %%i IN (*) do (
		...
		echo !BS!
		echo !ESC![0KChecking for files 50MB and over^.!ESC![0K
		echo !BS!
		echo !ESC![0KThis may take a few minutes ^. ^. ^.  !spinner!!ESC![0K
		echo !BS!

		if !size! GEq 49000000 (
			set /a fiftymbcount += 1
			echo !ESC![0KFile^: !fname!!ESC![0K
			echo !ESC![0KSize^: !size!!ESC![0J
			echo !fname! ^- !size! >> !reportfsize!.txt
		)
    ...
    echo !BS!
    echo !BS!
	echo !ESC![0K!fiftymbcount! files found at or over 50mb.!ESC![0K
	echo !fiftymbcount! files found at or over 50mb. >> !reportfsize!.txt
	...
	echo\  >> !reportfsize!.txt
	echo\  >> !reportfsize!.txt
	echo Files found at or over 100MB^: >> !reportfsize!.txt
	echo ^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^= >> !reportfsize!.txt
	echo\  >> !reportfsize!.txt
	...
	for /r %%i IN (*) do (
		...
		echo !ESC![0KChecking for files 100MB and over^.!ESC![0K
		echo !BS!
		echo !ESC![0KThis may take a few minutes ^. ^. ^.  !spinner!!ESC![0K
		echo !BS!

		if !size! GEq 99000000 (
			set /a hundredmbcount += 1
			echo !ESC![0KFile^: !fname!!ESC![0K
			echo !ESC![0KSize^: !size!!ESC![0J
			echo !fname! ^- !size! >> !reportfsize!.txt
		)
    ...
    echo !BS!
    echo !ESC![0K!fiftymbcount! files found at or over 50mb.!ESC![0K
	echo !ESC![0K!hundredmbcount! files found at or over 100mb.!ESC![0K
	echo !hundredmbcount! files found at or over 100mb. >> !reportfsize!.txt
	echo !BS!
	
	timeout /T 2 >nul

	echo\  >> !reportfsize!.txt
	echo End of report. >> !reportfsize!.txt

	echo !ESC![0KReport generated.!ESC![0K
	echo !ESC![0KSee !reportfsize!.txt for more details.!ESC![0K

	echo !BS!
	echo !ESC![0KPress any key to return to the main menu . . .!ESC![0K
	pause >nul
goto MainMenu
...
:FileTypeAudit
cls
	title STL Bitz Box Setup - File Type Audit . . .
	
	set /a "objcount=0"
	set /a "fbxcount=0"

	set "reportftype=reports\Report_FileType"
	del "!reportftype!.txt" >nul 2>&1
	
	echo Beginning file type audit^.!ESC![^?25l

	set "spinner=|"
	
	echo ^.OBJ files found^: >> !reportftype!.txt
	echo ^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^= >> !reportftype!.txt
	echo\  >> !reportftype!.txt

	echo\
	echo !ESC![0KChecking for .OBJ files . . .  !spinner!!ESC![0K
	echo !ESC![H
	for /r %%i IN (*.obj) do (
		echo %%i >> !reportftype!.txt
		set /a objcount += 1
	...
	echo ^.FBX files found^: >> !reportftype!.txt
	echo ^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^=^= >> !reportftype!.txt
	echo\  >> !reportftype!.txt

	echo\
	echo\
	echo !ESC![0KChecking for .FBX files . . .  !spinner!!ESC![0K
	echo !ESC![H
	for /r %%i IN (*.fbx) do (
		echo %%i >> !reportftype!.txt
		set /a fbxcount += 1
	...
	echo End of report. >> !reportftype!.txt

	echo\
	echo\
	echo\
	echo\
	echo Found !objcount! ^.OBJ files needing conversion.
	echo Found !fbxcount! ^.FBX files needing conversion.
	echo\
	echo Report generated.
	echo See !reportftype!.txt for more details.
...
:OBJtoSTL
cls
	title STL Bitz Box Setup - OBJ to STL Converter
...
:RebuildThumbs
cls
	title STL Bitz Box Setup - Thumbnail Generator
...
cls
	...
	echo\
	echo Press any key to continue to the DB rebuild . . .
	pause >nul
goto RebuildDB
...
cls
	...
	title STL Bitz Box Setup - Started: %time:~,5% ^| Est. run-time: !esthr!h !estmin!m
	echo\
	timeout /T 1 >nul
	...
	title STL Bitz Box Setup - Finalising...
	...
	echo\
	echo Press any key to launch the STL library . . .
	pause >nul
